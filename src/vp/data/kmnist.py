...
from vp.data.utils import RotationTransform, get_loader, get_subset_data
...
class KMNIST(torch.utils.data.Dataset):
    def __init__(
        ...
        self.dataset = torchvision.datasets.KMNIST(
            root=self.path, train=train, download=download
        )

        if len(classes) >= 10 and n_samples_per_class is None:
            self.data, self.targets = self.dataset.data, self.dataset.targets
        else:
            self.data, self.targets = get_subset_data(
                self.dataset.data,
        ...
        self.targets = torch.nn.functional.one_hot(
            torch.tensor(self.targets), len(classes)
        ).numpy()
...
def get_kmnist(
    ...
    test_loader = get_loader(
        dataset_test, batch_size=batch_size, shuffle=shuffle, drop_last=True, seed=seed
    )
